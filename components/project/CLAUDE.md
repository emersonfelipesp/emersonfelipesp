...
- `DemoInitRunner.tsx` — Client component. Standalone `Run it!` / `stop` / `replay` surface for the `nbx demo init` demo, with the same outer chrome as `InstallSimulator` plus a collapse/expand arrow. Wraps `sims/DemoInitTrace` for the streamed body. Mounted directly below the install simulator on the netbox-sdk page.
- `DemoDevicesListRunner.tsx` — Client component. Same standalone pattern as `DemoInitRunner` but for `nbx dcim devices list --help`. Wraps `sims/DemoDevicesList` (line-by-line streamed rendering of the captured docgen stdout). Mounted directly below `DemoInitRunner`.
